...
import random
from draw_distrib import draw_distrib
...
def generate_tf_record(path_images, path_to_csv, image_rate, cut_rate, boat_rate, tfrecord_dir):
    ...
    :param path_images: str, répertoire où sont stockées les images
    :param path_to_csv: str, chemin vers le csv qui doit être au format Pascal VOC
    ...
    :param cut_rate: float, pourcentage d'images utilisées pour formerle tfrecord train. Le tfrecord test sera sont formé avec les images restantes
    ...
    df_test = pd.DataFrame(columns=df.columns.tolist())
        ...
        # afin d'avoir des échantillons représentatifs de la base complète, 
